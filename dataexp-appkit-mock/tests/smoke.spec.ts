...
test('smoke test - app loads and displays main page', async ({ page }) => {
  await page.goto('/');

  await expect(
    page.getByRole('heading', { name: 'Cosmo 顧客データエクスポート' }),
  ).toBeVisible();
  await expect(page.getByRole('heading', { name: 'フィルタ' })).toBeVisible();
  await expect(page.getByText('データソース:')).toBeVisible();
  await expect(page.getByText('全件数')).toBeVisible();
  await expect(page.getByText('抽出件数')).toBeVisible();
  await expect(page.getByText('出力列数')).toBeVisible();
  await expect(page.getByRole('heading', { name: 'プレビュー' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'エクスポート' })).toBeVisible();
});

test('smoke test - export and save controls render', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByText('フォーマット')).toBeVisible();
  await expect(page.getByText('この条件を保存')).toBeVisible();
});
