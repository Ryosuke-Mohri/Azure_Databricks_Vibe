# VS Code + Claude Code + Databricks Agent Skills セットアップ手順 v3

Azure Databricks のサービングエンドポイント（Claude モデル）を、VS Code 拡張機能 **Claude Code** から利用し、**Databricks Agent Skills** をインストールするまでの手順書です。

> **v3 更新内容（v2 からの変更点）**
> - `CLAUDE_CODE_GIT_BASH_PATH` の確認方法を補足（`git.exe` と同じフォルダに `bash.exe` がある）
> - **Step 5：Skills のインストールは Git Bash で実行する手順に変更**（cmd / PowerShell では HTTP 400 で失敗しやすいため）
...
> 💡 **`bash.exe` は、普段使っている `git.exe` と同じフォルダ（`...\Git\bin\`）にあります。**
> 皆さんの環境（ユーザーインストール）の場合、`git.exe` が
> `C:\Users\<ユーザー名>\AppData\Local\Programs\Git\bin\git.exe`
> にあるので、`bash.exe` も**同じ `...\AppData\Local\Programs\Git\bin\` フォルダ内**にあります。
> （`git.exe` の場所が分かれば、その `git.exe` を `bash.exe` に読み替えたパスを設定すれば OK です。）
...
## 5. Databricks Agent Skills をインストールする

Claude Code が Databricks の各機能（SQL、Unity Catalog、Apps 等）を正しく扱えるよう、**スキル（知識ファイル群）をプロジェクトフォルダにインストール**します。

> ⚠️ **インストールは「Git Bash」で実行してください。**
> コマンドプロンプト / PowerShell で一括インストールを実行すると、社内プロキシの挙動により
> `HTTP 400`（`failed to fetch ... : HTTP 400`）で失敗することがあります。
> **Git Bash で実行すると一発で成功します。**

### 5-1. プロジェクトフォルダを作成する（任意）

**フォルダ名・場所は任意ですが、自分が管理できる場所にしてください。**（このフォルダがハンズオン中の作業フォルダになります）

例として Windows エクスプローラーや任意の方法で
`C:\Users\<ユーザー名>\vibecoding` のようなフォルダを作成しておきます。

### 5-2. Git Bash で Skills をインストールする

#### ① Git Bash アプリを開く

Windows の検索ボックスで「**Git**」と検索すると **Git Bash** が出てきます。これを起動します。
...
```bash
cd /c/Users/11250150/vibecoding
```

> ※ 上記は Windows 表示で `C:\Users\11250150\vibecoding` を指します。
> Git Bash ではパス区切りが `/`（スラッシュ）になり、`C:\` は `/c/` と表記します。

#### ③ インストールを実行する

移動先のフォルダで、以下をそのまま実行します。
...
> また、コマンドプロンプト / PowerShell で `HTTP 400` が出て `0/30` になった場合は、**Git Bash で 5-2 を実行し直してください。**
...
プロジェクトフォルダに移動してから実行してください（Git Bash 推奨）：
...
| Skills インストールが `HTTP 400` で失敗する（cmd / PowerShell） | **Git Bash で 5-2 を実行し直す。** Git Bash なら一発で成功する（cmd / PowerShell は社内プロキシの挙動で間欠的に 400 になる） |
| Skills インストールが `HTTP 400` で失敗する（CLI が古い） | Databricks CLI が古い（v1.1.x のバグ）。上記でアップデート後に再実行 |
| `X/30 skills installed` で止まっている | Git Bash で再実行。それでもダメなら CLI をアップデート、または `--skills <スキル名>` で個別インストール |
...
| Bash 関連エラー | `CLAUDE_CODE_GIT_BASH_PATH` が実際の `bash.exe` のパスと一致しているか確認（`where.exe bash` で確認可）。`bash.exe` は `git.exe` と同じ `...\Git\bin\` フォルダにある |
