...
test('smoke test - export page loads with filters and export action', async ({ page }) => {
  await page.goto('/');

  await expect(page.getByRole('heading', { name: '顧客データエクスポート' })).toBeVisible();
  await expect(page.getByText('training.dsg_vibe.customers')).toBeVisible();

  await expect(page.getByRole('heading', { name: 'フィルター' })).toBeVisible();
  await expect(page.getByLabel('氏名・顧客ID検索')).toBeVisible();
